...
{
    public class TankTicketPdfRenderer2
    {
        ...
        public byte[] Render(TankTicket tankTicket, string logoPath, int plantCode)
        {
            ...
            using (MemoryStream ms = new MemoryStream())
            {
                ...
                doc.Open();
                doc.AddTitle(string.Format("Tank Ticket ID # {0}", tankTicket.Ticket_Number));
        ...
        private void RenderLocationPertamina(PdfPCell leftCell, TankTicket tankTicket, Tank tank, Product product, string logoPath, int plantCode)
        {
            ...
            try
            {
                ...
                var header = FontFactory.GetFont("Helvetica", 12, Font.BOLD);
                // Menginisialisasi font Times New Roman
                BaseFont baseFont = BaseFont.CreateFont(BaseFont.TIMES_ROMAN, BaseFont.CP1252, BaseFont.NOT_EMBEDDED);
                Font theaderFont = new Font(baseFont, 12, Font.BOLD);
                Font headerFont = new Font(baseFont, 12, Font.BOLDITALIC);
                Font headerlabel = new Font(baseFont, 8, Font.NORMAL);
                ...
                PdfPCell headerValue = new PdfPCell();

                leftCell.PaddingTop = 10;

                Paragraph headerText = new Paragraph();
                if (tankTicket.Operation_Type == "BLD")
                {
                    headerText = new Paragraph("Untuk Menghitung Mutasi (BLD) Blending", headerFont);
                }
                if (tankTicket.Operation_Type == "DL")
                {
                    headerText = new Paragraph("Untuk Menghitung Mutasi (DL) Idle Mode", headerFont);
                }
                if (tankTicket.Operation_Type == "ICR")
                {
                    headerText = new Paragraph("Untuk Menghitung Mutasi (ICR) Issue to Crude to Prod", new Font(baseFont, 10, Font.BOLDITALIC));
                }
                if (tankTicket.Operation_Type == "ILS")
                {
                    headerText = new Paragraph("Untuk Menghitung Mutasi (ILS) Issue to LSTK", headerFont);
                }
                if (tankTicket.Operation_Type == "IOT")
                {
                    headerText = new Paragraph("Untuk Menghitung Mutasi (IOT) Issue Other", headerFont);
                }
                if (tankTicket.Operation_Type == "IS")
                {
                    headerText = new Paragraph("Untuk Menghitung Mutasi (IS) Issue to Sales", headerFont);
                }
                if (tankTicket.Operation_Type == "IST")
                {
                    headerText = new Paragraph("Untuk Menghitung Mutasi (IST) Issue to Stock Transfer", new Font(baseFont, 10, Font.BOLDITALIC));
                }
                if (tankTicket.Operation_Type == "OWN")
                {
                    headerText = new Paragraph("Untuk Menghitung Mutasi (OWN) Own Use", headerFont);
                }
                if (tankTicket.Operation_Type == "PI")
                {
                    headerText = new Paragraph("Untuk Menghitung Mutasi (PI) Physical Inventory", headerFont);
                }
                if (tankTicket.Operation_Type == "RCR")
                {
                    headerText = new Paragraph("Untuk Menghitung Mutasi (RCR) Crude Receipt (PTEP/REF)", new Font(baseFont, 9, Font.BOLDITALIC));
                }
                if (tankTicket.Operation_Type == "ROT")
                {
                    headerText = new Paragraph("Untuk Menghitung Mutasi (ROT) Receipt Others", headerFont);
                }
                if (tankTicket.Operation_Type == "RPR")
                {
                    headerText = new Paragraph("Untuk Menghitung Mutasi (RPR) Production Receipt (REF)", new Font(baseFont, 9, Font.BOLDITALIC));
                }
                if (tankTicket.Operation_Type == "TPL")
                {
                    headerText = new Paragraph("Untuk Menghitung Mutasi (TPL) Pipeline Transfer", new Font(baseFont, 10, Font.BOLDITALIC));
                }
                if (tankTicket.Operation_Type == "TRF")
                {
                    headerText = new Paragraph("Untuk Menghitung Mutasi (TRF) Transfer Others", headerFont);
                }
                if (tankTicket.Operation_Type == "TT")
                {
                    headerText = new Paragraph("Untuk Menghitung Mutasi (TT) Tank to Tank Transfer", new Font(baseFont, 10, Font.BOLDITALIC));
                }
                if (tankTicket.Operation_Type == "UDG")
                {
                    headerText = new Paragraph("Untuk Menghitung Mutasi (UDG) Up/Down Gradation", new Font(baseFont, 10, Font.BOLDITALIC));
                }
                if (tankTicket.Operation_Type == "CHK")
                {
                    headerText = new Paragraph("Untuk Menghitung Mutasi (CHK) Stock Checking", new Font(baseFont, 10, Font.BOLDITALIC));
                }

                headerText.SpacingBefore = 20;
                headerText.SpacingAfter = 5;
                headerText.Alignment = Element.ALIGN_LEFT;
                leftCell.AddElement(headerText);

                // Buat tabel dengan dua kolom
                PdfPTable table = new PdfPTable(2);
                table.WidthPercentage = 100;

                // Set lebar kolom: kolom pertama akan lebih lebar dari kolom kedua
                float[] widths = new float[] { 40f, 60f };
                table.SetWidths(widths);

                // Tambahkan sel pertama dengan teks "TANK TICKET"
                PdfPCell leftcell = new PdfPCell(new Phrase("TANK TICKET", theaderFont));
                leftcell.Border = Rectangle.NO_BORDER;
                leftcell.HorizontalAlignment = Element.ALIGN_LEFT;
                table.AddCell(leftcell);

                // Tambahkan sel kedua dengan teks "NO : " + tankTicket.Ticket_Number
                PdfPCell rightCell = new PdfPCell(new Phrase("NO : " + tankTicket.Ticket_Number, theaderFont));
                rightCell.Border = Rectangle.NO_BORDER;
                rightCell.HorizontalAlignment = Element.ALIGN_RIGHT;
                table.AddCell(rightCell);

                // Atur spasi dan perataan
                table.SpacingBefore = 10;
                table.SpacingAfter = 5;

                // Tambahkan tabel ke sel
                ...
                {
                    headerLabel = new PdfPCell(new Phrase("\n " + "    OPEN\n", label));
                    headerTable.AddCell(headerLabel);
                    headerLabel.HorizontalAlignment = 1;

                }

                if (tankTicket.Operation_Status == 2)
                {
                    headerLabel = new PdfPCell(new Phrase("\n " + "   CLOSE\n", label));
                    headerTable.AddCell(headerLabel);
                    headerLabel.HorizontalAlignment = 1;
                ...
                headerTable.AddCell(headerValue);
                ...
                formLabel = new PdfPCell(new Phrase("NO", label));
                ...
                formLabel = new PdfPCell(new Phrase("TANK NO", label));
                tInner2.AddCell(formLabel);
                formValue = new PdfPCell(new Phrase(tank.Tank_Name, value));
                tInner2.AddCell(formValue);
                formLabel = new PdfPCell(new Phrase("OPERATION FROM/TO *", label));
                tInner2.AddCell(formLabel);
                formValue = new PdfPCell(new Phrase(tankTicket.Operation_Type, value));
                tInner2.AddCell(formValue);
                formLabel = new PdfPCell(new Phrase("TYPE OF PRODUCT", label));
                tInner2.AddCell(formLabel);
                ...
                tInner1.AddCell(formLabel);
                formValue = new PdfPCell(new Phrase(tank.Blank.ToString(), value));
                //formValue = new PdfPCell(new Phrase(tankData.PointDeskHeight.ToString(), value));
                formValue.HorizontalAlignment = 2;
                tInner1.AddCell(formValue);
                formLabel = new PdfPCell(new Phrase("mm", label));
                ...
                tInner1.AddCell(formLabel);
                formValue = new PdfPCell(new Phrase((tankTicket.WaterLevel?.ToString() ?? "0"), value));
                formValue.HorizontalAlignment = 2;
                tInner1.AddCell(formValue);
                formLabel = new PdfPCell(new Phrase("mm", label));
                formLabel.HorizontalAlignment = 1;
                ...
                tInner1.AddCell(formLabel);
                formValue = new PdfPCell(new Phrase((tankTicket.LiquidLevel?.ToString() ?? "0"), value));
                formValue.HorizontalAlignment = 2;
                tInner1.AddCell(formValue);
                formLabel = new PdfPCell(new Phrase("mm", label));
                formLabel.HorizontalAlignment = 1;
                ...
                formLabel = new PdfPCell(new Phrase("MAT. TEMP °C/°P", label));
                tInner1.AddCell(formLabel);
                ...
                formLabel = new PdfPCell(new Phrase("TEST. TEMP °C/°P", label));
                tInner1.AddCell(formLabel);
                formValue = new PdfPCell(new Phrase(tankTicket.TestTemperature.ToString("F"), value));
                formValue.HorizontalAlignment = 2;
                tInner1.AddCell(formValue);
                formLabel = new PdfPCell(new Phrase("", label));
                formLabel.HorizontalAlignment = 1;
                ...
                formLabel = new PdfPCell(new Phrase("S.G / DENSITY", label));
                tInner1.AddCell(formLabel);
                formValue = new PdfPCell(new Phrase((tankTicket.LiquidDensity.HasValue ? tankTicket.LiquidDensity.Value.ToString("0.000") : "0.000"), value));
                formValue.HorizontalAlignment = 2;
                tInner1.AddCell(formValue);
                formLabel = new PdfPCell(new Phrase("", label));
                formLabel.HorizontalAlignment = 1;
        ...
        private void RenderForOther(PdfPCell leftCell, TankTicket tankTicket, Tank tank, Product product, string logoPath)
        {
            ...
            tInner1.AddCell(formLabel);
            formValue = new PdfPCell(new Phrase((tankTicket.LiquidTemperature?.ToString("F") ?? "0"), value));
            formValue.HorizontalAlignment = 2;
            tInner1.AddCell(formValue);
        ...
        private void RenderForUser(PdfPCell leftCell, TankTicket tankTicket, Tank tank, Product product, string logoPath)
        {
            ...
            tInner1.AddCell(CreateFieldValue((tankTicket.WaterLevel?.ToString() ?? "0")));
            tInner1.AddCell(CreateFieldValue("mm"));
            ...
            tInner1.AddCell(CreateFieldValue((tankTicket.LiquidTemperature?.ToString() ?? "0")));
